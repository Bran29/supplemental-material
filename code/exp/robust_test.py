...
def data_normal_2d(orign_data,dim="col"):
    ...
    if d_min.shape[0] == orign_data.shape[0]:
        ...
        dst = dst.unsqueeze(0)
    norm_data = torch.sub(orign_data,d_min).true_pide(dst)
    return norm_data
...
def compute_robust_volumes_ref(X_tildes, dcube_collections):
...
for d in ds:
    for s in s_values:
        cluster = min(int(math.pow(2,d)),s-1)
        distortions_mine = []
        distortions_ref = []
        s = int(s)
        for _ in tqdm(range(M)):
            # S1 = rand((s, d)) * (1 - 0) + 0
            # # S2 = rand((s, d)) * (1 - 0) + 0
            # S1 = np.random.uniform(0, 0.1, (s, d))
            # S2 = np.random.uniform(0, 1, (s, d))
            # S1 = np.repeat(np.array([[0, 1], [10, 10]]),50,axis=0)
            # S2 =  np.repeat(np.array([[9.9,9.8], [10,10]]),50,axis=0)
            S1 = randn((s, d)) * (0.2 - 0) + 0.8
            S2 = randn((s, d)) * (0.3- 0) + 0.2
            scaler = MinMaxScaler()
            S1 = scaler.fit_transform(S1)
            S2 = scaler.fit_transform(S2)
            # S1 = torch.from_numpy(S1)
            # S2 = torch.from_numpy(S2)
            # # # S2 = randn((s, d)) * (0.3 - 0) + 0.2
            # # S1 = F.normalize(S1, p=2, dim=0)
            # # S2 = F.normalize(S2, p=2, dim=0)
            # S1=S1.numpy()
            # S2=S2.numpy()
            v1, v2, v_all = compute_volume(S1, S2, d)

            X_tilde1, cubes1,dist1,score1= compute_X_tilde_and_counts_cluster(S1, cluster)
            X_tilde2, cubes2,dist2,score2 = compute_X_tilde_and_counts_cluster(S2, cluster)

            # d1, d2, d_all = compute_volume(dist1, dist2, d)

            rv1,rv2 = compute_robust_volumes_mine(X_tilde1, cubes1,X_tilde2, cubes2,s)
            # test d
            # rv1*=d1
            # rv2*=d2
            distortions_mine.append((v1/v2) / (rv1/rv2))
            ...
            X_tilde, cubes = compute_X_tilde_and_counts(S2, omega=0.5)
            ...
            distortions_ref.append((v1 / v2) / (rv1 / rv2))
...
#             S2 = torch.from_numpy(S2)
#
#             X_tilde, cubes = compute_X_tilde_and_counts(S1, omega=0.2)
#             rv1 = compute_robust_volumes_ref([X_tilde], [cubes])[0]
#
#             X_tilde, cubes = compute_X_tilde_and_counts(S2, omega=0.2)
#             rv2 = compute_robust_volumes_ref([X_tilde], [cubes])[0]
#
...
plt.ylabel("Bounded Distortion$", fontsize=30)
...
np.save("../datasave/distortion_mean_mine.npy", distortion_mean_mine)
np.save("../datasave/distortion_std_mine.npy", distortion_std_mine)
np.save("../datasave/distortion_mean_ref.npy", distortion_mean_ref)
np.save("../datasave/distortion_std_ref.npy", distortion_std_ref)
