...
def compute_losses_high_or_low_add(feature_datasets, labels, feature_datasets_test, test_labels, M, omega, high_or_low,K_cluster,args):
    ...
    if args.method=="mine":
        ...
        # robust_volumes=[]
        # Xtildes, cluster,dist,score = zip(
        #     *(compute_X_tilde_and_counts_cluster(dataset.numpy(), k_cluseter=K_cluster) for dataset in feature_datasets_copy))
        # Xtildes, cluster_collections = list(Xtildes), list(cluster)
        # for i in range(len(Xtildes)):
        #     robust_volumes.append( compute_robust_volumes_mine(Xtildes[i], cluster_collections[i],Xtildes[i].shape[0]))
        # robust_volumes=np.array(robust_volumes)
    ...
    for i in range(len(feature_datasets)):
        # validition set
        dataset = TensorDataset(feature_datasets_test_copy[idx[i]], test_labels_copy[idx[i]])
        valid_loader = DataLoader(dataset, batch_size=1000)
        #training set
        dataset = TensorDataset(feature_datasets_copy[idx[i]], labels_copy[idx[i]])
        train_loader = DataLoader(dataset, batch_size=64)
        # initial model
        torch.manual_seed(args.seed)
        model = NN_Classification(1, args.output_dim, device=args.device)
        optimizer = torch.optim.SGD(model.parameters(), lr=0.01,momentum=0.9)
        ...
        train_losses.append(train_loss)
        test_losses.append(valid_loss)
        # record parameter after training
        for para in model.parameters():
            temp = copy.deepcopy(para)
            temp = temp.detach()
            if temp.dim() == 2:
...
def main():
    ...
    for i in tqdm(range(runs)):
        ...
        #     combines]
        #
        feature_datasets = [feature_datasets[0][:, combines[i]].view(-1, 1) for i in range(5)]
        feature_datasets_test = [feature_datasets_test[0][:, combines[i]].view(-1, 1) for i in range(5)]

        labels = [labels[0] for i in range(5)]
        test_labels = [test_labels[0] for i in range(5)]


        if exp_type == 'add':
            high_test, high_train, high_param_variation, high_robust_volumes, high_idx = compute_losses_high_or_low_add(
                feature_datasets, labels, feature_datasets_test, test_labels, M, omega=0.1, high_or_low='high',
                K_cluster=cluster_num, args=args)

            low_test, low_train, low_param_variation, low_robust_volumes, low_idx = compute_losses_high_or_low_add(
                feature_datasets, labels, feature_datasets_test, test_labels, M, omega=0.1, high_or_low='low',
                K_cluster=cluster_num, args=args)

            rand_test, rand_train, rand_param_variation, rand_robust_volumes, rand_idx = compute_losses_high_or_low_add(
                feature_datasets, labels, feature_datasets_test, test_labels, M, omega=0.1, high_or_low='random',
                K_cluster=cluster_num, args=args)
        if exp_type == 'pv':
    ...
    np.savez(
        '../datasave/{}_valuable_{}_{}_{}_{}M_class_save_volume.npz'.format(exp_type, function, method, cluster_num, M),
        high_test=high_test_arr, high_train=high_train_arr, high_volume=high_robust_volumes,
        low_volume=low_robust_volumes, rand_volume=rand_robust_volumes, low_test=low_test_arr,
        low_train=low_train_arr, rand_test=rand_test_arr, rand_train=rand_train_arr,
        high_pv_train=high_pv_train_arr, low_pv_train=low_pv_train_arr, rand_pv_train=rand_pv_train_arr, M=M, D=D,
