...
for d in ds:
    for s in s_values:
        ...
        for _ in tqdm(range(M)):
            ...
            norepli_S1 = randn((s-int(0.1*(s)), d)) * (0.2 - 0) + 0.8
            norepli_S2 = randn((s-int(0.1*(s)), d)) * (0.8 - 0) + 0.2

            repli_S1 = norepli_S1[random.randint(0,norepli_S1.shape[0]-1),:]
            repli_S2 =  norepli_S2[random.randint(0,norepli_S2.shape[0]-1),:]
            repli_S1 = repli_S1.repeat(int(0.1*(s)), 1)
            repli_S2 = repli_S2.repeat(int(0.1*(s)), 1)

            S1=torch.vstack([norepli_S1,repli_S1])
            S2 = torch.vstack([norepli_S2, repli_S2])

            scaler = MinMaxScaler()
            S1 = scaler.fit_transform(S1)
            S2 = scaler.fit_transform(S2)

            v1, v2, v_all = compute_volume(S1[0:s - int(0.1 * (s)),:], S2[0:s - int(0.1 * (s)),:], d)
...
np.save("../datasave/distortion_mean_mine_r.npy", distortion_mean_mine)
np.save("../datasave/distortion_std_mine_r.npy", distortion_std_mine)
np.save("../datasave/distortion_mean_ref_r.npy", distortion_mean_ref)
np.save("../datasave/distortion_std_ref_r.npy", distortion_std_ref)
