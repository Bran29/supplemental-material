...
df_noutlies = df[(z<3).all(axis=1)]
df_outlies = df[(z>=3).any(axis=1)]
X=df_noutlies.drop(columns=["quality","pH"])
y=df_noutlies [["pH"]]
...
X_outlies=df_outlies.drop(columns=["quality","pH"]).to_numpy()
y_outlies=df_outlies [["pH"]].to_numpy()

X_train200=X_train[0:100].to_numpy()
X_train_out1=np.vstack((X_train200,X_outlies[0:1]))
X_train_out2=np.vstack((X_train200,X_outlies[0:5]))
y_train200=y_train[0:100].to_numpy()
y_train_out1=np.vstack((y_train200,y_outlies[0:1]))
y_train_out2=np.vstack((y_train200,y_outlies[0:5]))

feature_datasets=[X_train200,X_train_out1,X_train_out2]
label_datasets=[y_train200,y_train_out1,y_train_out2]
...
def shapley_volume(Xs, omega=0.1):
    ...
    for ordering_count, ordering in enumerate(orderings):
        ...
        for position, i in enumerate(ordering):
            ...
            #cluster-based
            robust_volumes = []
            Xtildes, cluster, dist, score = compute_X_tilde_and_counts_cluster_origin(curr_train_X, k_cluseter=2)
...
np.savez('../datasave/res_{}_{}D_{}M{}_outlies_minmax_10_w0.5.npz'.format("wine", D, M, suffix),
