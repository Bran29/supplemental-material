...
def load_uber_lyft(n_participants=3, s=30, reduced=False, path_prefix=''):
    ...
    df = pd.read_csv('../data/Uber_lyft/rideshare_kaggle.csv')
    
    # Remove empty cell rows
    df = df[df['price'].isnull() == False]
    
    # Feature selection
    df = df[['price', 'distance', 'surge_multiplier',  'windBearing', 'cloudCover']]
...
def load_house_kc(n_participants=3, s=30, reduced=False, path_prefix=''):
    """
	    Method to load the Uber_lyft dataset.
        Args:
            n_participants (int): number of data subsets to generate
    ...
    df = pd.read_csv('../data/House_sale_in_King_Country/kc_house_data.csv')
    ...
    # df = df[['price', 'bedrooms ', 'surge_multiplier', 'windBearing', 'cloudCover']]
    # # df = pd.get_dummies(df,columns=['name'], drop_first=True)
    # # df = df.drop(['name_Lux', 'name_Lux Black', 'name_Lux Black XL', 'name_Lyft', 'name_Lyft XL'], axis=1)
    ...
    for i in range(n_participants):
        ...
        feature_datasets_test.append(torch.from_numpy(np.array(X_test[test_start_idx:test_end_idx], dtype=np.float32)))
        test_labels.append(
            torch.from_numpy(np.array(y_test[test_start_idx:test_end_idx], dtype=np.float32).reshape(-1, 1)))

    return feature_datasets, labels, feature_datasets_test, test_labels

def load_breast_cancer(n_participants=3, s=30, train_test_diff_distr=False, path_prefix=''):
    df = pd.read_csv('../data/breast_cancer/Breast_cancer_data.csv')
    df = df[df['diagnosis'].isnull() == False]
